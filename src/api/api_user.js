...
async function getSubscribesGamesByQueries(data) {
    ...
    return (await axios.post(`/api/user-following-query/${data.id}?scores=${data.scores}&developers=${data.developers}&genres=${data.genres}`, data, config)).data;
}
...
async function getFriendsList(data) {
    ...
    return (await axios.get(`/api/friends/${data.id_user}`, config)).data;
}

async function handlerFriendRequest(data) {
    const config = {
        headers: {
            Authorization: `${data.token}`
        }
    ...
    return (await axios.post(`/api/friend`, data, config)).data;
}
